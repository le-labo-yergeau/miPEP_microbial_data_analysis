...
biom_file<-import_biom(here("data", "ITS", "otu_table_filtered.biom"), here("data", "ITS", "tree.fasttree"), here("data", "ITS", "otus.fasta"), parseFunction=parse_taxonomy_greengenes)
biom_file
```
...
```{r}
sample<- data.frame(fread(here("data", "ITS", "mapping_file.tsv"), sep="\t"), check.names=F)
head(sample)
...
p<-p + labs(title="Rarefaction curve", subtitle =  "miPEP_H ITS samples")
...
```{r}
otu_biom <- import_biom(here("data", "ITS", "otu_table_absolute_ITS.biom1"))

tax <- tax_table(tax_table_phylo_filtered) #taxa hasn't changed

samples <- sample_data(merge) #use your metadata from before

phylo_abs_ITS <- merge_phyloseq(tax,otu_biom,samples)

#Before continuing, we must discard the samples R2c and R7c from the root samples, because no valid Ct was measured in order to quantify the number of ITS copies in these samples.
Sample_toRemove <- c("X.SampleID"="cR2", "cR7","ctrlS6")
...
phylo_abs_ITS #should have only 93 samples now (-cR2 and cR7 and ctrls6)
...
phylo_unplanted <- subset_samples(phylo_abs_ITS, Compartment=="SOL.NU")
```
...
# ps1.com.fam <- aggregate_top_taxa(pseq, "Phylum", top = 10) : function is deprecated
...
StackedBarPlot_phylum <- taxa_abundance_table_phylum_roots %>% 
  ...
  ) + niceTheme + scale_fill_manual(values = c("darkorchid", "darkgreen", "deeppink","brown1","dodgerblue3", "darkorange1","darkseagreen"))
...
StackedBarPlot_genus_roots <- taxa_abundance_table_genus_roots %>% 
  ...
  theme(
    ...
    legend.text = element_text(size = 10),
    strip.text = element_text(size = 12)
  )+ niceTheme + scale_fill_manual(values = c("darkblue", "darkgoldenrod1","darkgrey", "darkseagreen", "darkorchid", "darkolivegreen1", "lightskyblue", "darkgreen", "deeppink", "khaki2"))
...
ggplot(data = ., aes(x = Type, y = Abundance)) +
  ...
  geom_jitter(aes(color = OTU), height = 0, width = .2) +
  labs(x = "", y = "Abundance\n") +
  facet_wrap(~ OTU, scales = "free") + guides(col = FALSE) +theme_classic() +theme(axis.text.x=element_text(size=8, angle=45,hjust=1)) + ggtitle(label = "Abundance of top fungal phyla in unplanted soils")
...
ggplot(data = ., aes(x = Type, y = Abundance)) +
  ...
  geom_jitter(aes(color = OTU), height = 0, width = .2) +
  labs(x = "", y = "Abundance\n") +
  facet_wrap(~ OTU, scales = "free") + guides(col = FALSE) +theme_classic() +theme(axis.text.x=element_text(size=8, angle=45,hjust=1)) + ggtitle(label = "Abundance of fungal genera in unplanted soils")
```
...
```{r}
ggplot(data = data.frame("total_reads" =  phyloseq::sample_sums(phylo_abs_ITS),
"observed" = phyloseq::estimate_richness(phylo_abs_ITS, measures = "Observed")[, 1]),
aes(x = total_reads, y = observed)) + geom_point() + geom_smooth(method="lm", se = FALSE)+ labs(x = "\nTotal Reads", y = "Observed Richness\n")

#Draw the rarefaction curve for phylo_abs_ITS

#remotes::install_github("mahendra-mariadassou/phyloseq-extended", ref = "dev")
library(phyloseq.extended)

p<-ggrare(phylo_abs_ITS, step=100, color = "Compartment", label = "X.SampleID", se = FALSE)
p<-p + labs(title="Rarefaction curve", subtitle =  "miPEP ITS samples")
p<-p+scale_color_manual(values = c("dark blue", "dark orange", "dark red"))
p<-p+theme_minimal()
...
min_reads_soil
#[1] 764 048 corresponds to aS4, the next lowest is at 1 254 140 aS2: this will be the rarefaction limit, to not loose any more samples. The two higest samples are up to 20M.

reads_roots <- sample_sums(phylo_roots)
min_reads_roots <- min(reads_roots)
min_reads_roots
#[1] 2525 corresponds to bR7, the next lowest is 31 281 (bR3) so it will be the rarefaction limit.

reads_unplanted <- sample_sums(phylo_unplanted)
min_reads_unplanted <- min(reads_unplanted)
min_reads_unplanted
# 502744 = ctrlSN2,keep this rarefaction limit as many are around that number.
...
# Roots samples 
phylo_rar_roots <- phyloseq::rarefy_even_depth(phylo_roots, sample.size = 31281, rngseed = 711, replace = FALSE)
...
# Soil samples
phylo_rar_soil <- phyloseq::rarefy_even_depth(phylo_soil, sample.size = 1254140 , rngseed = 711, replace = FALSE)

testit(10)
# unplanted samples
phylo_rar_unplanted <- phyloseq::rarefy_even_depth(phylo_unplanted, sample.size = 502744 , rngseed = 711, replace = FALSE)
...
library(readr)
indice_diversite_roots <-read_delim("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/indice_diversite_roots_1.txt","\t", escape_double = FALSE, trim_ws = TRUE)
...
library(readr)
indice_diversite_soil <-read_delim("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/indice_diversite_soil_1.txt","\t", escape_double = FALSE, trim_ws = TRUE)
...
hist(fit2$residuals) #suit une loi normale?
shapiro.test(fit2$residuals)
glm_fit2 <- glm(Shannon~Type, data=indice_diversite_soil) #link="inverse" by default
summary(glm_fit2)
...
library(readr)
indice_diversite_unplanted <-read_delim("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/indice_diversite_unplanted_1.txt","\t", escape_double = FALSE, trim_ws = TRUE)
...
hist(fit1$residuals) #suit une loi normale?
shapiro.test(fit1$residuals)# si p-value > 0,05 alors la distribution n'est pas significativement différente d'une distribution normale
glm_fit1 <- glm(Observed~Type, data=indice_diversite_unplanted) #link="inverse" by default
summary(glm_fit1)

fit2 <-lm(Shannon~Type, data=indice_diversite_unplanted)
hist(fit2$residuals) #suit une loi normale?
shapiro.test(fit2$residuals)
glm_fit2 <- glm(Shannon~Type, data=indice_diversite_unplanted) #link="inverse" by default
summary(glm_fit2)

fit3 <-lm(Simpson~Type, data=indice_diversite_unplanted)
hist(fit3$residuals) #suit une loi normale? if not normal try a GLM
shapiro.test(fit3$residuals)
glm_fit3 <- glm(Simpson~Type, data=indice_diversite_unplanted) #link="inverse" by default
summary(glm_fit3)
```
...
#This graph shows that the first 3 axes are ok
...
bc_roots <- bc_roots + ggtitle("Bray-Curtis distances based PCoA of roots and close rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
bc_roots
...
bc_roots_nb  + ggtitle("Bray-Curtis distances based PCoA of roots and close rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
...
screeplot(rda, bstick=TRUE)
#This graph shows that the 3 first axes are > random
...
bc_soil <- bc_soil + ggtitle("Bray-Curtis distances based PCoA of distant rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
bc_soil
...
bc_soil_nb <- bc_soil_nb  + ggtitle("Bray-Curtis distances based PCoA of distant rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
bc_soil_nb
...
bc_unplanted <- bc_unplanted + ggtitle("Bray-Curtis distances based PCoA of unplanted soil fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
bc_unplanted
...
bc_unplanted_nb <- bc_unplanted_nb  + ggtitle("Bray-Curtis distances based PCoA of unplanted soil fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
bc_unplanted_nb
...
tree <- read.tree(here("data", "ITS", "tree.fasttree"))
tree
...
new.tree
#use the latest phyloseq info (-c2R, c7R, ctrlS6)
...
WUF_roots <- WUF_roots + ggtitle("Weighted UniFrac distances based PCoA of roots and close rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
WUF_roots
...
WUF_roots_nb <- WUF_roots_nb  + ggtitle("Weighted UniFrac distances based PCoA of roots and close rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
...
WUF_soil <- WUF_soil + ggtitle("Weighted UniFrac distances based PCoA of distant rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
WUF_soil
...
WUF_soil_nb <- WUF_soil_nb  + ggtitle("Weighted UniFrac distances based PCoA of distant rhizospheric fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
...
WUF_unplanted <- WUF_unplanted + ggtitle("Weighted UniFrac distances based PCoA of unplanted soil fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
WUF_unplanted
...
WUF_unplanted_nb <- WUF_unplanted_nb  + ggtitle("Weighted UniFrac distances based PCoA of unplanted soil fungal communities") +theme(plot.title = element_text(size = 8, face = "bold"))
...
res_roots_miPEPa_order = resLFC[order(resLFC$padj, na.last=NA), ] #69
sigtab_roots_miPEPa = res_roots_miPEPa_order[which(res_roots_miPEPa_order$padj < alpha),] #28 ASVs
sigtab_roots_miPEPa_top_50 <- sigtab_roots_miPEPa[c(1:28),] 
...
plot_miPEPa_roots_order <- plot_miPEPa_roots + scale_x_discrete(
  labels=my_labels) + scale_color_manual(values = c( "darkorchid","lightskyblue","darkgreen", "deeppink","darksalmon"))
...
res_roots_miPEPb_order = resLFC[order(resLFC$padj, na.last=NA), ] #64 ASVs
sigtab_roots_miPEPb = res_roots_miPEPb_order[which(res_roots_miPEPb_order$padj < alpha),] #24 ASVs
sigtab_roots_miPEPb_top_50 <- sigtab_roots_miPEPb[c(1:24),] 
...
plot_miPEPb_roots_order <- plot_miPEPb_roots + scale_x_discrete(
  labels=my_labels) + scale_color_manual(values = c("darkorchid","deeppink","darkgreen","lightskyblue", "darksalmon"))
...
res_roots_miPEPc_order = resLFC[order(resLFC$padj, na.last=NA), ] #59
sigtab_roots_miPEPc = res_roots_miPEPc_order[which(res_roots_miPEPc_order$padj < alpha),] #20
sigtab_roots_miPEPc_top_50 <- sigtab_roots_miPEPc[c(1:20),] 
...
plot_miPEPc_roots_order <- plot_miPEPc_roots + scale_x_discrete(
  labels=my_labels) + scale_color_manual(values = c("deeppink","darkorchid","darkgreen","lightskyblue", "darksalmon"))
...
res_soil_miPEPa_order = resLFC[order(resLFC$padj, na.last=NA), ] #89
sigtab_soil_miPEPa = res_soil_miPEPa_order[which(res_soil_miPEPa_order$padj < alpha),] #18 ASVs
sigtab_soil_miPEPa_top_50 <- sigtab_soil_miPEPa[c(1:18),] 
...
res_soil_miPEPb_order = resLFC[order(resLFC$padj, na.last=NA), ] #77 ASVs
sigtab_soil_miPEPb = res_soil_miPEPb_order[which(res_soil_miPEPb_order$padj < alpha),] #11 ASVs
sigtab_soil_miPEPb_top_50 <- sigtab_soil_miPEPb[c(1:11),] 
...
res_soil_miPEPc_order = resLFC[order(resLFC$padj, na.last=NA), ] #86
sigtab_soil_miPEPc = res_soil_miPEPc_order[which(res_soil_miPEPc_order$padj < alpha),] #15
sigtab_soil_miPEPc_top_50 <- sigtab_soil_miPEPc[c(1:15),] 
...
res_unplanted_miPEPa_order = resLFC[order(resLFC$padj, na.last=NA), ] #89
sigtab_unplanted_miPEPa = res_unplanted_miPEPa_order[which(res_unplanted_miPEPa_order$padj < alpha),] #22 ASVs
sigtab_unplanted_miPEPa_top_50 <- sigtab_unplanted_miPEPa[c(1:22),] 
...
res_unplanted_miPEPb_order = resLFC[order(resLFC$padj, na.last=NA), ] #93 ASVs
sigtab_unplanted_miPEPb = res_unplanted_miPEPb_order[which(res_unplanted_miPEPb_order$padj < alpha),] #26 ASVs
sigtab_unplanted_miPEPb_top_50 <- sigtab_unplanted_miPEPb[c(1:26),] 
...
res_unplanted_miPEPc_order = resLFC[order(resLFC$padj, na.last=NA), ] #81
sigtab_unplanted_miPEPc = res_unplanted_miPEPc_order[which(res_unplanted_miPEPc_order$padj < alpha),] #17
sigtab_unplanted_miPEPc_top_50 <- sigtab_unplanted_miPEPc[c(1:17),] 
...
```{r}
#miPEP-A
row.names(sigtab_roots_miPEPa)[row.names(sigtab_roots_miPEPa) %in% row.names(sigtab_unplanted_miPEPa)] #None

#miPEP-B
row.names(sigtab_roots_miPEPb)[row.names(sigtab_roots_miPEPb) %in% row.names(sigtab_unplanted_miPEPb)] #100
phylo_roots@tax_table["100",] #"Fungi" "Ascomycota" "Dothideomycetes" "Pleosporales" "Pleosporales_fam_Incertae_sedis" "Pyrenochaeta"

#miPEP-C
row.names(sigtab_roots_miPEPc)[row.names(sigtab_roots_miPEPc) %in% row.names(sigtab_unplanted_miPEPc)] #297, 194
phylo_roots@tax_table["194",] #"Fungi" "Ascomycota" "Leotiomycetes" "Helotiales" "Hyaloscyphaceae" "Hyaloscypha"
phylo_roots@tax_table["297",] #"Fungi" "Basidiomycota" "Microbotryomycetes" "Sporidiobolales" "Sporidiobolales_fam_Incertae_sedis" "Rhodotorula"

#Overlap between miPEPs
row.names(sigtab_roots_miPEPa)[row.names(sigtab_roots_miPEPa) %in% row.names(sigtab_roots_miPEPb)] #"277"  "102"  "144"  "403"  "354"  "131"  "399"  "501"  "1232" "1274" "174"  "214"
row.names(sigtab_roots_miPEPa)[row.names(sigtab_roots_miPEPa) %in% row.names(sigtab_roots_miPEPc)] #"315"  "277"  "102"  "144"  "403"  "354"  "131"  "390"  "399"  "501"  "1232" "1274" "174"  "648"
row.names(sigtab_roots_miPEPb)[row.names(sigtab_roots_miPEPb) %in% row.names(sigtab_roots_miPEPc)] #"194"  "102"  "114"  "144"  "354"  "131"  "243"  "464"  "399"  "297"  "1232" "1274" "174"  "501"  "277" "403" 
...
attr(root.venn, "intersections")$`A:B:C` #"277"  "102"  "144"  "403"  "354"  "131"  "399"  "501"  "1232" "1274" "174"

#Shared taxonomy
phylo_roots@tax_table["102",]
phylo_roots@tax_table["131",]
phylo_roots@tax_table["144",]
phylo_roots@tax_table["174",]
phylo_roots@tax_table["277",]
phylo_roots@tax_table["354",]
phylo_roots@tax_table["399",]
phylo_roots@tax_table["403",]
phylo_roots@tax_table["501",]
phylo_roots@tax_table["1232",]
phylo_roots@tax_table["1274",]
...
attr(unplanted.venn, "intersections")$`A:B:C` #59

#Shared taxonomy
phylo_unplanted@tax_table["59",]
...
#OTU_table_phylo_roots_1<-read_excel("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/OTU_table_phylo_roots_1.xlsx")
...
#OTU_table_phylo_soil_1<-read_excel("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/OTU_table_phylo_soil_1.xlsx")
...
#(proba que le groupe investigué appartienne bien au groupe target, du fait que cette espèce indic soit dedans),
# B= fidelity or sensitivity: (proba qu'une espèce indic appartienne bien au groupe target)
...
#OTU_table_phylo_unplanted_1<-read_excel("C:/Users/hmiddleton/Dropbox/Projet_Mutants_miPEPs_miRNA/Harriet/Metabarcoding_mutants_and_miPEP/Analyses_métabarcoding/miPEP_H/ITS/OTU_table_phylo_unplanted_1.xlsx")
...
Etienne's simplified graphs
```{r}
pseq_roots <- phylo_roots %>% aggregate_taxa(level = "Class")
pseq_unplanted <- phylo_unplanted %>% aggregate_taxa(level = "Class")
...
sample.root.s <- sample.root.s[-c(18,23),] #Two samples that were removed from the OTU table cR2 and cR7
...
phy.map.root.long <- gather(phy.map.root,Class,relabund,5:15) #transform in long format for ggplot
...
phy.map.unplanted.long <- gather(phy.map.unplanted,Class,relabund,5:15) #transform in long format for ggplot
...
StackedBarPlot_phylum_rel_EY <- ggplot(phylum.long, aes(x =Type, y = relabund, fill = Class)) +
  ...
  theme_bw()+
scale_fill_manual(values = c("darkblue", "darkgoldenrod1","darkgrey", "darkseagreen", "darkorchid", "darkolivegreen1", "lightskyblue", "darkgreen", "deeppink", "khaki2", "firebrick"), labels = c("Agaricomycetes", "Chytridiomycetes", "Dothideomycetes", "Eurotiomycetes", "Leotiomycetes", "Mucoromycotina", "Others", "Saccharomycetes", "Sordariomycetes", "Tremmellomycetes", "Unclassified"))

StackedBarPlot_phylum_rel_EY

#Anovas
#Root
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Agaricomycetes"),])) #7.715 0.000758 ***
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Chytridiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Dothideomycetes"),])) #7.977 0.000622 ***
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Eurotiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Leotiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Mucoromycotina_cls_Incertae_sedis"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Saccharomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Sordariomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="RACINE" & phylum.long$Class=="Tremellomycetes"),])) #NS

#Unplanted
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Agaricomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Chytridiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Dothideomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Eurotiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Leotiomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Mucoromycotina_cls_Incertae_sedis"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Saccharomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Sordariomycetes"),])) #NS
summary(aov(relabund~Type, data = phylum.long[(phylum.long$Compartment=="SOL.NU" & phylum.long$Class=="Tremellomycetes"),])) #NS
...
# Test normality
shapiro.test(div.table.shannon[div.table.shannon$Compartment=="RACINE",]$value) #W = 0.91006, p-value = 0.0172
shapiro.test(div.table.shannon[div.table.shannon$Compartment=="SOL.NU",]$value) #W = 0.93812, p-value = 0.06622
...
summary(anova.div.root) #F=3.704, P=0.0248
TukeyHSD(anova.div.root) #ctrl: a, A: ab, B: b, C:ab
summary(anova.div.unplanted) #F=0.731 P=0.542
tuk.labels.div <- data.frame(x=c(1,2,3,4), y=c(rep(3.75, 4)), Compartment="RACINE", symbol=c("a","ab","b","ab")) #To add letters on the graph
...
qPCR.s$Sample==row.names(sample.s) #Sort check: 96 TRUE
qPCR.sample <- cbind(sample.s,qPCR.s)

# Test normality
shapiro.test(qPCR.sample[qPCR.sample$Compartment=="RACINE",]$ITS) #W = 0.8942, p-value = 0.006075
shapiro.test(qPCR.sample[qPCR.sample$Compartment=="SOL.NU",]$ITS) #W = 0.95076, p-value = 0.1514

#Anova
anova.ITS.root <- aov(ITS~Type, data = qPCR.sample[qPCR.sample$Compartment=="RACINE",])
anova.ITS.unplanted <- aov(ITS~Type, data = qPCR.sample[qPCR.sample$Compartment=="SOL.NU",])

summary(anova.ITS.root) #F=4.618, P=0.0102
TukeyHSD(anova.ITS.root) #ctrl:a, A:ab, B:b, C:ab
summary(anova.ITS.unplanted) #F=3.717, P=0.0228
TukeyHSD(anova.ITS.unplanted) #ctrl: a, A: b, B: ab, C:ab
tuk.labels.ITS <- data.frame(x=c(rep(c(1,2,3,4),2)), y=c(rep(1e+06, 8)), Compartment=c(rep("RACINE",4), rep("SOL.NU",4)), symbol=c("a","ab","b","ab", "a","b","ab","ab")) #To add letters on the graph
...
#ITS
fun.qPCR.box <- ggplot(qPCR.sample[qPCR.sample$Compartment!="SOL",], aes(x=Type, y=ITS)) +
  geom_boxplot() +
  geom_point()+
  geom_text(data = tuk.labels.ITS, aes(x=x, y=y, label=symbol), size=5)+
  facet_grid(~Compartment, labeller = labeller(Compartment = comp.labs))+
  theme_bw() + 
  ylab("ITS region copies (per ng DNA)") + theme(legend.position="none")+
  xlab("Treatment")+
  scale_x_discrete(labels=c("Water", "miPEP-A", "miPEP-B", "miPEP-C"))+
  scale_y_log10()
fun.qPCR.box
...
Fig2 <- ggarrange(StackedBarPlot_phylum_rel_EY,p_Div,fun.qPCR.box, ncol = 1, legend = "bottom", labels = c("A","B","C"))
Fig2
ggsave(filename = here("output", "Fig2.tiff"),  plot = Fig2, compression = "lzw", height = 14, units = "in")
